...
question_content="""Given a list paths of directory info, including the directory path, and all
...
class Solution:
    def findDuplicate(self, paths: List[str]) -> List[List[str]]:
        def getPathAndContent(s):
            ...
            # print(ans)
            path = ans.pop(0)
            paths_contents = [[path + '/' + ans[2*i], ans[2*i+1]] for i in range(len(ans)//2)]
            # for i in paths_contents:
            #     print(i)
            return paths_contents

        paths_contents = getPathAndContent(paths)
